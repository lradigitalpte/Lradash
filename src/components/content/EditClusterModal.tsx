...
export function EditClusterModal({ cluster, onOpenChange, onSuccess }: EditClusterModalProps) {
  const { projectId } = useParams()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState("")
  const [formData, setFormData] = useState({
    name: cluster.name,
    status: cluster.status as "planning" | "building" | "active" | "strong",
    authorityScore: cluster.authorityScore || 0,
    pillarPageUrl: cluster.pillarPageUrl || "",
    subtopics: (cluster.subtopics || []).join(", ")
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const subtopicsArray = (formData.subtopics as string)
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s.length > 0)
      ...
      const response = await apiClient.put(
        `/projects/${projectId}/marketing/clusters/${cluster._id}`,
        payload
      )

      if (!response.ok) {
        throw new Error((await response.json())?.message || "Failed to update cluster")
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 p-6 dark:border-slate-800">
          <h2 className="text-lg font-black text-slate-900 dark:text-white">Edit Topic Cluster</h2>
          <button
