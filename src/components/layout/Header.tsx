...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full shrink-0 border-b border-border/80 bg-background/90 shadow-sm backdrop-blur-md supports-backdrop-filter:bg-background/85 dark:border-slate-800/90 dark:bg-slate-950/90 dark:shadow-[0_1px_0_0_rgba(255,255,255,0.06)]">
      <div className="mx-auto flex h-14 max-w-[1920px] items-center gap-3 px-3 sm:gap-4 sm:px-5">
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          <SidebarTrigger className="size-9 shrink-0 rounded-lg border border-border/80 bg-card text-muted-foreground shadow-sm transition-colors hover:bg-muted hover:text-foreground" />
          <Separator orientation="vertical" className="hidden h-6 sm:block" />
          <div className="hidden min-h-9 min-w-0 flex-1 items-center rounded-lg border border-border/70 bg-muted/40 px-3 py-1.5 md:flex md:max-w-xl">
            <Breadcrumbs />
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          <div
            className={cn(
              ...
              searchOpen ? "w-[min(100%,18rem)]" : "w-9"
            )}
          >
            {searchOpen ? (
              <div className="relative">
                ...
                <input
                  ...
                  className="h-9 w-full rounded-lg border border-border bg-background/80 pr-3 pl-9 text-xs shadow-sm transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30"
                  onBlur={() => {
                    setSearchOpen(false)
                  }}
                />
              ...
              <Button
                variant="ghost"
                size="icon"
                className="size-9 rounded-lg border border-transparent text-muted-foreground hover:border-border hover:bg-muted hover:text-foreground"
                onClick={() => {
                  setSearchOpen(true)
                }}
              >
          ...
          <div className="flex items-center gap-0.5 rounded-xl border border-border/60 bg-muted/40 p-1 shadow-sm ring-1 ring-black/[0.03] dark:ring-white/[0.06]">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="relative size-9 rounded-lg text-muted-foreground hover:bg-background/80 hover:text-foreground"
                >
                  <Bell className="size-4" />
                  {unreadCount > 0 && (
                    <span className="absolute top-1.5 right-1.5 flex h-4 min-w-4 items-center justify-center rounded-md bg-primary px-0.5 text-[10px] font-semibold text-primary-foreground shadow-sm">
            ...
            <Separator orientation="vertical" className="mx-0.5 h-6 bg-border/70" />
            <Button
              variant="ghost"
              size="icon"
              className={cn(
                ...
                calendarActive
                  ? "border border-primary/25 bg-primary/12 text-primary shadow-inner"
                  : "border border-transparent text-muted-foreground hover:bg-background/90 hover:text-foreground"
              )}
              asChild
            >
              <Link href="/calendar" aria-label="Calendar" title="Calendar">
                <CalendarDays className="size-[1.125rem]" strokeWidth={calendarActive ? 2.25 : 2} />
              </Link>
            </Button>
            <Separator orientation="vertical" className="mx-0.5 h-6 bg-border/70" />
            <ThemeToggle />
            <Separator orientation="vertical" className="mx-0.5 h-6 bg-border/80" />
            <div className="pr-0.5 pl-0.5">
              <UserNav />
            </div>
          </div>
        </div>
