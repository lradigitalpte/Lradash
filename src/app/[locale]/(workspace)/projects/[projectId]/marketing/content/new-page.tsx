...
const STRATEGY_TYPES = {
  organic: { icon: "🌱", label: "Organic Growth", color: "emerald" },
  ads: { icon: "📢", label: "Paid Ads", color: "blue" },
  viral: { icon: "⚡", label: "Viral Content", color: "yellow" },
  community: { icon: "👥", label: "Community", color: "purple" },
  influencer: { icon: "⭐", label: "Influencer", color: "pink" },
  analytics: { icon: "📊", label: "Analytics", color: "indigo" }
}
  ...
  const [strategies, setStrategies] = useState<SocialStrategy[]>([
    {
      ...
      type: "organic",
      status: "active",
      description: "Build authority through consistent LinkedIn publishing and industry insights",
      implementation: ["3x weekly posts", "Engage 10+ accounts daily", "Share case studies"],
      targetReach: 50000,
      actualReach: 28500,
      targetEngagement: 2500,
      actualEngagement: 1850,
      roi: 85,
      platforms: ["LinkedIn"],
    ...
    {
      id: "2",
      name: "Twitter Viral Campaigns",
      type: "viral",
      status: "in_progress",
      description: "Launch trending hashtags for product launches and announcements",
      implementation: ["Weekly trending hashtag", "Engage 500+ tweets", "Run polls"],
      targetReach: 100000,
      actualReach: 45200,
      targetEngagement: 5000,
      actualEngagement: 2340,
      roi: 62,
      platforms: ["Twitter"],
    ...
    },
    {
      ...
      type: "ads",
      status: "success",
      description: "Drive qualified leads through search and display advertising",
      implementation: ["High-intent keywords", "A/B test pages", "Optimize bids"],
      targetReach: 75000,
      actualReach: 82500,
      targetEngagement: 2000,
      actualEngagement: 2850,
      roi: 234,
  ...
  return (
    <div className="space-y-8 p-8 pb-20">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-lg shadow-slate-200/20 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
          <p className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Avg ROI</p>
          <p className="mt-2 text-3xl font-black text-purple-600 dark:text-purple-400">{avgROI}%</p>
        </div>
      </div>
      ...
      <div className="rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-2xl shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        ...
        {strategies.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-12 text-center dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <Button className="mt-4 rounded-xl bg-purple-600 px-6 text-[11px] font-bold tracking-widest text-white uppercase hover:bg-purple-700">
      ...
      {/* Tips */}
      <div className="rounded-[2.5rem] border border-slate-100 bg-gradient-to-br from-slate-50 to-slate-100 p-8 dark:border-slate-800 dark:from-slate-900 dark:to-slate-800">
        <div className="flex gap-4">
