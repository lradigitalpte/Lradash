...
export default function MarketingStrategyPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await apiClient.post("/api/marketing/strategies", {
        projectId,
        title,
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-white p-6 dark:bg-slate-950">
          ...
          <div className="flex h-full items-center justify-center">
            <Card className="w-full max-w-sm rounded-2xl p-8 text-center">
              ...
              <p className="mb-6 text-sm text-slate-500">
                Create your first strategy to start tracking
              </p>
              <Button
                onClick={handleCreate}
            ...
            <div className="space-y-2 overflow-y-auto pr-1">
              {strategies.map((s) => (
                <Card
                  ...
                  className={cn(
                    "cursor-pointer rounded-xl p-3 transition-all hover:shadow-md",
                    selectedStrategy?._id === s._id
              ...
              <div className="col-span-2 space-y-3 overflow-y-auto pr-1">
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <CardDescription className="text-[9px] capitalize">
                          {selectedStrategy.platform} {selectedStrategy.status}
                        </CardDescription>
                      </div>
                      <Button
                        variant="outline"
                  ...
                  <Card className="shrink-0 rounded-2xl">
                    ...
                    <CardContent className="grid grid-cols-2 gap-2 pt-0">
                      {[
                        {
                          label: "Reach",
                          target: selectedStrategy.targets.reach,
                          actual: tR,
                          color: "bg-blue-500"
                        },
                        {
                          label: "Likes",
                          target: selectedStrategy.targets.likes,
                          actual: tL,
                          color: "bg-red-500"
                        },
                        {
                          label: "Shares",
                          target: selectedStrategy.targets.shares,
                          actual: tS,
                          color: "bg-sky-400"
                        },
                        {
                          label: "Comments",
                          target: selectedStrategy.targets.comments,
                          actual: tC,
                          color: "bg-green-500"
                        }
                      ].map((item) => (
                        ...
                        >
                          ...
                          <div className="mt-1 h-1.5 rounded-full bg-slate-200">
                            <div
                              ...
                              style={{ width: `${pct(item.actual, item.target)}%` }}
                            />
                          </div>
                          <div className="mt-0.5 text-[8px] text-slate-400">
                            {item.actual} / {item.target} ({pct(item.actual, item.target)}%)
                          </div>
                        </div>
                      ))}
                    </CardContent>
                ...
                <Card className="shrink-0 rounded-2xl">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    ...
                    <Button
                      onClick={() => {
                        setShowLogResults(true)
                      }}
                      size="sm"
                      className="h-auto rounded-lg bg-emerald-600 px-2 py-1 text-xs hover:bg-emerald-700"
                    >
                        ...
                        {selectedStrategy.engagementLogs.map((log, i) => (
                          ...
                          >
                            ...
                            <div className="flex gap-3">
                              ...
                              <span className="flex items-center gap-0.5 text-xs font-bold text-purple-500">
                ...
                <Card className="shrink-0 rounded-2xl">
                  ...
                  <CardContent className="grid grid-cols-4 gap-2 pt-0">
                    <div className="rounded-lg bg-red-50 p-2 text-center dark:bg-red-950">
                      <Heart className="mx-auto mb-0.5 h-3 w-3 text-red-500" />
                      <div className="text-[8px] tracking-widest text-slate-500 uppercase">
                        Likes
                      </div>
                      <div className="text-sm font-black text-red-600">{tL}</div>
                    </div>
                    <div className="rounded-lg bg-blue-50 p-2 text-center dark:bg-blue-950">
                      <Share2 className="mx-auto mb-0.5 h-3 w-3 text-blue-500" />
                      <div className="text-[8px] tracking-widest text-slate-500 uppercase">
                        Shares
                      </div>
                      <div className="text-sm font-black text-blue-600">{tS}</div>
                    </div>
                    <div className="rounded-lg bg-green-50 p-2 text-center dark:bg-green-950">
                      <MessageCircle className="mx-auto mb-0.5 h-3 w-3 text-green-500" />
                      <div className="text-[8px] tracking-widest text-slate-500 uppercase">
                        Comments
                      </div>
                      <div className="text-sm font-black text-green-600">{tC}</div>
                    </div>
                    <div className="rounded-lg bg-purple-50 p-2 text-center dark:bg-purple-950">
                      <TrendingUp className="mx-auto mb-0.5 h-3 w-3 text-purple-500" />
                      <div className="text-[8px] tracking-widest text-slate-500 uppercase">
                        Reach
                      </div>
                      <div className="text-sm font-black text-purple-600">{tR}</div>
                    </div>
                  </CardContent>
                </Card>
              </div>
      ...
      {selectedStrategy && (
        <>
          <SetTargetsModal
            open={showSetTargets}
            onOpenChange={setShowSetTargets}
            strategyId={selectedStrategy._id}
            currentTargets={selectedStrategy.targets}
            onTargetsUpdated={fetchStrategies}
          />
          <LogResultsModal
            open={showLogResults}
            onOpenChange={setShowLogResults}
            strategyId={selectedStrategy._id}
            onLogAdded={fetchStrategies}
          />
        </>
      )}
    </div>
